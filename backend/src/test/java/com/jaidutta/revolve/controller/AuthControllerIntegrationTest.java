...
public class AuthControllerIntegrationTest {
    ...
    @Nested class RegistrationTests {
        ...
        @Test void should_returnBadRequest_when_registeringDuplicateUsername() {
            registerUser("username", "password"); // First registration should succeed
            ResponseEntity<ApiResponseDto> response2 = registerUser("username",
        ...
        void should_returnBadRequest_when_registeringDuplicateUsernameButWithCapitalisedLetter() {
            registerUser("username", "password"); // First registration should succeed
            ResponseEntity<ApiResponseDto> response2 = registerUser("USERNAME",
                                                                    "password"); // Second attempt with different case

            // Use helper for BAD_REQUEST related to the 'username' field
            assertErrorResponse(response2, HttpStatus.BAD_REQUEST, "username");
        }

        @Test void should_returnBadRequest_when_passwordTooShort() {
        ...
        @Test void should_returnBadRequest_when_passwordTooLong() {
            ResponseEntity<ApiResponseDto> response = registerUser("username",
                                                                   "passwordpasswordpasswordpassword-passwordpasswordpasswordpassword");
            // Use helper for BAD_REQUEST related to the 'password' field (validation)
            assertErrorResponse(response, HttpStatus.BAD_REQUEST, "password");
        }

        @Test void should_returnBadRequest_when_usernameTooLong() {
            ResponseEntity<ApiResponseDto> response = registerUser(
                    "usernameusernameusernameusername-usernameusernameusernameusername",
                    "password");
        ...
        @Test void should_returnBadRequest_when_usernameTooShort() {
        ...
        @Test void should_returnBadRequest_when_usernameIsBlank() {
        ...
        @Test void should_returnBadRequest_when_usernameIsWhitespace() {
            ResponseEntity<ApiResponseDto> response = registerUser("   ", "password");
    ...
    @Nested class LoginTests {
        @Test void should_returnOkStatus_when_loggingInToAccountThatExists() {
        ...
        @Test void should_returnUnauthorized_when_loggingInToAccountThatDoesNotExist() {
            ResponseEntity<ApiResponseDto> response = loginUser("nonexistentuser",
                                                                "password"); // Use a clearly non-existent user
        ...
        @Test void should_returnUnauthorized_when_loggingInToAccountWitEmptyPassword() {
            // Registering a user is needed to test authentication failure
            registerUser("username", "password");
            ResponseEntity<ApiResponseDto> response = loginUser("username",
                                                                ""); // Test with the correct username, empty password

            // Call the helper method
            assertErrorResponse(response, HttpStatus.UNAUTHORIZED, "auth.error");
        }

        @Test void should_returnUnauthorized_when_loggingInToAccountWitEmptyUsername() {
            // Registering a user is needed to test authentication failure
            registerUser("username", "password");
            ResponseEntity<ApiResponseDto> response = loginUser("",
                                                                "password"); // Test with empty username, correct password
