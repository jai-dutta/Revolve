...
public class RecurringActivityService {
    ...
    @Transactional
    public RecurringActivity createRecurringActivity(User user,
                                                     RecurringActivityDto recurringDto) throws TooManyRecurringActivitiesRegisteredByUserException {
        // Todo: add config instead of magic number 20
        if (user.getEventsCurrentlyRegistered() > 20) {
            throw new TooManyRecurringActivitiesRegisteredByUserException("Too many recurring " +
                    "activities registered for user: " + user.getUsername());
        }
    ...
    @Transactional
    public void deleteRecurringActivityByIdAndUser(Long id, User user) throws RecurringActivityNotFoundException {
    ...
    public RecurringActivity updateRecurringActivityByIdAndUser(Long id, User user,
                                            RecurringActivityDto recurringDto) throws RecurringActivityNotFoundException {
        RecurringActivity existingActivity =
                getRecurringActivityByIdAndUser(id, user)
                        .orElseThrow(() -> new RecurringActivityNotFoundException("Activity " + id +
                                " not found"));
